...
def extract_step_info(logprob_tokens, logprobs, divergence_type, ans_solution):
    step_token_prob = []
    # total confidence
    step_confidence = []
...
def token_efficient_reasoning(seed):
    ...
    arg_parse.add_argument("--engine", type=str, default="gpt-4")
    ...
    arg_parse.add_argument("--api_key", type=str, default=None, required=True)
    ...
    origin_file = f"{output_dir}/major.ori.{engine}.temp{str(temp)}.{str(seed)}.{diver}.txt"
    rethink_file = f"{output_dir}/major.rk.{engine}.temp{str(temp)}.{str(seed)}.{diver}.txt"
    ...
    with (open(origin_file, 'w', encoding="utf-8") as fo, open(rethink_file, 'w', encoding="utf-8") as fr):
        for idx, (ques, gold_ans) in tqdm(enumerate(zip(math_test_qs, math_test_ans)),
            ...
            step_info, pred_num = extract_step_info(logprob_tokens, logprobs, diver,ans_solution)
    ...
    overall_file = f"{output_dir}/major.overall.{engine}.temp{str(temp)}.{str(seed)}.{diver}.txt"
    visualization(origin_file, rethink_file, overall_file)
    error_analysis_file = f"{output_dir}/major.ea.{engine}.temp{str(temp)}.{str(seed)}.{diver}.txt"
    error_cases_visualization(origin_file, rethink_file, error_analysis_file)


if __name__ == "__main__":
    seed_list = [1357, 1122, 817, 42, 1213, 965, 626, 317]
